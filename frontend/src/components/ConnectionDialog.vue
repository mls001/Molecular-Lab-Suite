<template>
  <div v-if="visible" class="modal-overlay" @click.self="close">
    <div class="modal-content" @mousedown.stop @click.stop style="width:520px;background:var(--c-elev);border-radius:8px;padding:20px;box-shadow:0 4px 20px rgba(0,0,0,0.35);color:var(--c-text);">
      <h3 style="margin-top:0;">{{ $t('连接服务器') }}</h3>

      <!-- 预设管理 -->
      <div style="display:flex;gap:6px;margin-bottom:12px;flex-wrap:wrap;">
        <select class="control h-lg" style="flex:1;min-width:120px;" v-model="selectedPreset" @change="onPresetChange">
          <option value="">{{ $t('-- 加载预设 --') }}</option>
          <option v-for="name in connectionPresetNames" :key="name" :value="name">{{ name.replace(/^conn_/, '') }}</option>
        </select>
        <input class="control h-lg" style="width:120px;" v-model="newPresetName" :placeholder="$t('预设名称')" />
        <button class="btn btn-primary h-lg" @click="savePreset">{{ $t('保存') }}</button>
        <button class="btn btn-danger h-lg" @click="deletePreset" :disabled="!selectedPreset">{{ $t('删除') }}</button>
      </div>

      <!-- 连接参数 -->
      <div style="display:grid;grid-template-columns:70px 1fr;gap:8px;margin-bottom:12px;">
        <label>{{ $t('主机') }}</label>
        <input v-model="localHost" class="control h-lg" :placeholder="$t('IP地址')" />
        <label>{{ $t('端口') }}</label>
        <input v-model="localPort" class="control h-lg" placeholder="22" />
        <label>{{ $t('用户名') }}</label>
        <input v-model="localUsername" class="control h-lg" :placeholder="$t('用户名')" />
        <label>{{ $t('密码') }}</label>
        <input v-model="localPassword" class="control h-lg" type="password" :placeholder="$t('密码')" />
      </div>

      <!-- 按钮 -->
      <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px;">
        <button class="btn btn-primary" @click="doConnect" :disabled="connecting">
          {{ connecting ? (remoteStore.backendRetrying ? $t('连接中(等待后端启动)…') : $t('连接中…')) : $t('连接') }}
        </button>
        <button class="btn btn-default" @click="close" :disabled="connecting">{{ $t('取消') }}</button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async savePreset() {
      ...
      if (!name) {
        this.errorMessage = $tr('请输入预设名称')
        return
      }
      if (!this.localHost || !this.localUsername) {
        this.errorMessage = $tr('请填写主机和用户名')
        return
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.errorMessage = data.detail || $tr('保存失败')
        }
      } catch (e) {
        this.errorMessage = $tr('保存失败: {0}', { 0: e.message })
      }
    },
    ...
    async onPresetChange() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.errorMessage = data.detail || $tr('加载失败')
        }
      } catch (e) {
        this.errorMessage = $tr('加载失败: {0}', { 0: e.message })
      }
    },

    async deletePreset() {
      if (!this.selectedPreset) return
      if (!confirm($tr('确定删除预设 "{0}" 吗？', { 0: this.selectedPreset.replace(/^conn_/, '') }))) return
      try {
        ...
        if (response.ok) {
          ...
          this.errorMessage = data.detail || $tr('删除失败')
        }
      } catch (e) {
        this.errorMessage = $tr('删除失败: {0}', { 0: e.message })
      }
    },
